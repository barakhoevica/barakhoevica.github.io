  ...
  try {
    if (typeof supabaseClient !== 'undefined' && supabaseClient) {
      ...
      if(reviews && reviews.length){
        siteData.reviews = reviews.map(row => ({ id: row.id, text: row.text, rating: row.rating || 5 }));
...
function starsHtml(rating){
  ...
  let out = '';
  for(let i=1;i<=5;i++) out += `<span class="${i<=r?'':'dim'}">★</span>`;
  return out;
}

/* ---- reviews carousel ---- */
let reviewIdx = 0;
function renderReview(direction){
  ...
  if(direction === 'next') el.classList.add('dir-next');
  if(direction === 'prev') el.classList.add('dir-prev');
  setTimeout(() => {
    el.textContent = '«' + siteData.reviews[reviewIdx].text + '»';
    counter.textContent = (reviewIdx+1) + '/' + siteData.reviews.length;
    if(starsEl) starsEl.innerHTML = starsHtml(siteData.reviews[reviewIdx].rating);
    el.classList.remove('fading', 'dir-next', 'dir-prev');
  }, 200);
}
...
function renderGrid(){
  ...
  siteData.products.forEach((p, idx) => {
    ...
    tile.style.setProperty('--i', idx % 2);
    tile.innerHTML = `
...
function openReviewForm(){
  ...
  modal.innerHTML = `
    ...
    <div class="star-picker" id="publicStarPicker">
      ${[1,2,3,4,5].map(n => `<span class="star filled" data-n="${n}">★</span>`).join('')}
    </div>
    <div class="admin-form">
      ...
      <button class="primary ripple-wrap" id="submitReviewBtn">Отправить отзыв</button>
    </div>
  ...
  const picker = document.getElementById('publicStarPicker');
  const stars = picker.querySelectorAll('.star');
  const paint = (n) => stars.forEach(s => s.classList.toggle('filled', parseInt(s.getAttribute('data-n')) <= n));
  stars.forEach(s => {
    s.onclick = () => { selectedRating = parseInt(s.getAttribute('data-n')); paint(selectedRating); };
  });
  picker.dataset.getRating = '';
  picker._getRating = () => selectedRating;
}
...
async function submitPublicReview(){
  ...
  try {
    const picker = document.getElementById('publicStarPicker');
    const rating = picker && picker._getRating ? picker._getRating() : 5;
    const { data, error } = await supabaseClient.functions.invoke('submit-review', {
      body: { text, hp: hpEl.value, rating }
    });
...
/* ---- единая точка входа для всех административных изменений ----
   ...
   Используется fetch(), а не supabaseClient.functions.invoke(), с явными
   заголовками apikey/Authorization — admin-action развёрнута с
   verify_jwt = false (см. supabase/config.toml), так как токен
   подписан нашим собственным JWT_SECRET, а не секретом Supabase Auth. */
async function adminAction(action, payload){
  ...
  const url = `${SUPABASE_URL}/functions/v1/admin-action`;
  let res, data;
  try {
    res = await fetch(url, {
      method: 'POST',
    ...
    console.error('[adminAction] сетевая ошибка:', networkErr);
    throw networkErr;
  }

  if(res.status === 401 || res.status === 403){
    console.warn('[adminAction] сервер отклонил токен (status', res.status, '):', data.error);
    clearAdminToken();
    adminUnlocked = false;
    renderAdminPassScreen();
  ...
  if(!res.ok || data.success === false){
    console.error('[adminAction] ошибка операции:', data.error);
...
function adminTabsHtml(){
  const tabs = [['stock','Наличие'],['reviews','Отзывы'],['products','Товары'],['stats','Статистика']];
...
function renderAdminPanel(){
  ...
  else if(adminTab === 'stats') body = adminStatsData ? adminStatsHtml() : '<p style="color:var(--ink-soft);">Загрузка статистики...</p>';
  ...
  if(adminTab === 'products'){
    initProductDragSort(panel);
  }
  if(adminTab === 'stats' && !adminStatsData){
    loadAdminStats();
  }
}
...
function adminStatsHtml(){
  return `
    <div class="admin-note-box">Статистика считается только по нажатиям кнопки «Войти» на предупреждении о чтении Корана. Сторонние сервисы аналитики не используются — данные хранятся только в вашей базе Supabase.</div>
    <div class="stats-grid">
      <div class="stats-card"><div class="num">${adminStatsData.total}</div><div class="lbl">Всего входов</div></div>
      <div class="stats-card"><div class="num">${adminStatsData.today}</div><div class="lbl">Входов сегодня</div></div>
    </div>
  `;
}
...
async function loadAdminStats(){
  try {
    adminStatsData = await adminAction('stats.get', {});
  } catch(e){
    console.error(e);
    adminStatsData = { total: 0, today: 0 };
  }
  if(adminTab === 'stats') renderAdminPanel();
}
...
function adminReviewsHtml(){
  const list = adminReviewsData.map((r,i) => `
    <div class="review-item">
      <div class="txt">
        ...
        ${r.text}<span class="admin-status-pill ${r.status}">${r.status === 'pending' ? 'На модерации' : 'Опубликован'}</span>
      </div>
  ...
  return `
    ...
    <div class="admin-form">
      ...
      <div class="admin-rating-select" id="adminNewReviewStars">
        ${[1,2,3,4,5].map(n => `<span class="star filled" data-n="${n}">★</span>`).join('')}
      </div>
...
function adminProductsHtml(){
  const list = siteData.products.map((p,i) => `
    <div class="admin-form admin-product-item" data-id="${p.id}" style="border-bottom:1px solid rgba(43,38,32,0.1); padding-bottom:12px; margin-bottom:12px;">
      <div class="admin-product-row">
        <span class="drag-handle" title="Перетащить для сортировки">⠿</span>
        <img src="${p.img}" style="width:44px;height:44px;border-radius:8px;object-fit:cover;" loading="lazy">
  ...
  `).join('');
  return `
    <div class="admin-note-box">Изображение для нового товара указывается ссылкой (URL). Потяните за «⠿», чтобы изменить порядок товаров — новый порядок сразу сохранится и отразится на сайте.</div>
    ...
    <div id="sortableProductList">${list || '<p style="color:var(--ink-soft);">Нет товаров</p>'}</div>
  `;
}
...
function bindAdminTabEvents(panel){
  ...
  let adminNewReviewRating = 5;
  const adminStarPicker = panel.querySelector('#adminNewReviewStars');
  if(adminStarPicker){
    const stars = adminStarPicker.querySelectorAll('.star');
    stars.forEach(s => {
      s.onclick = () => {
        adminNewReviewRating = parseInt(s.getAttribute('data-n'));
        stars.forEach(x => x.classList.toggle('filled', parseInt(x.getAttribute('data-n')) <= adminNewReviewRating));
      };
    });
  }
  ...
  if(addReviewBtn) addReviewBtn.onclick = async () => {
    ...
    let data;
    try {
      data = await adminAction('reviews.create', { text: val, rating: adminNewReviewRating });
    } catch(e){ console.error(e); return; }
    // отзыв, добавленный админом, публикуется сразу
    siteData.reviews.push({ id: data.id, text: data.text, rating: data.rating || adminNewReviewRating });
  ...
  panel.querySelectorAll('.review-approve').forEach(btn => {
    btn.onclick = async () => {
      ...
      if(!siteData.reviews.some(r => r.id === rev.id)){
        siteData.reviews.push({ id: rev.id, text: rev.text, rating: rev.rating || 5 });
        renderReview();
      }
      renderAdminPanel();
      flashSaved();
...
function initProductDragSort(panel){
  const container = panel.querySelector('#sortableProductList');
  if(!container) return;
  ...
  container.querySelectorAll('.drag-handle').forEach(handle => {
    handle.addEventListener('pointerdown', (e) => {
      e.preventDefault();
      const item = handle.closest('.admin-product-item');
      if(!item) return;
      dragEl = item;
      ...
      const onMove = (ev) => {
        ...
        const siblings = Array.from(container.children).filter(x => x !== dragEl);
        for(const sib of siblings){
          const rect = sib.getBoundingClientRect();
          const mid = rect.top + rect.height / 2;
          const sibIndex = Array.from(container.children).indexOf(sib);
          const dragIndex = Array.from(container.children).indexOf(dragEl);
          if(pointerY < mid && sibIndex < dragIndex){
            container.insertBefore(dragEl, sib);
            dragEl.style.transform = 'translateY(0px)';
            startY = ev.clientY;
            break;
          } else if(pointerY > mid && sibIndex > dragIndex){
      ...
      const onUp = async () => {
        ...
        const byId = {};
        siteData.products.forEach(p => byId[p.id] = p);
        siteData.products = newOrderIds.map(id => byId[id]).filter(Boolean);
        renderGrid();

        try {
          await adminAction('products.reorder', { order: newOrderIds });
          flashSaved();
        } catch(err){ console.error('[products.reorder]', err); }
      };
...
/* ---- счётчик посещений (только Supabase, без сторонних сервисов) ----
   Считаем один вход = один клик по «Войти». submit-review/verify-admin
   не трогаем — это отдельная публичная функция track-visit. */
function trackVisit(){
  try {
    supabaseClient.functions.invoke('track-visit', { body: {} }).catch((e) => {
      console.warn('[trackVisit] не удалось записать посещение:', e);
    });
  } catch(e){
    console.warn('[trackVisit] ошибка:', e);
  }
}

/* ---- ripple-эффект на кнопках (чистый CSS/JS, без библиотек) ---- */
document.addEventListener('pointerdown', (e) => {
  const target = e.target.closest('.order-btn, .intro-btn, .review-add-btn, .admin-form button.primary, .nav-btn, .social-btn, .admin-tab, .admin-btn, .approve-btn');
  if(!target) return;
...
if('serviceWorker' in navigator){
  window.addEventListener('load', () => {
    navigator.serviceWorker.register('sw.js').catch((e) => {
      console.warn('[PWA] не удалось зарегистрировать service worker:', e);
    });
  });
}
